...
Note that \cref{c3_first} aligns with the definition provided in \cref{c3_chap2}, as $x_{uv} = 1$ implies that $u$ and $v$ should belong to \textit{different clusters}, while $x_{uv} = 0$ implies that the two vertices should be placed into the \textit{same cluster}.
