...
\begin{itemize}
    ...
    \item the oucomes from an ILP, identical to the one developed by \textcite{multi-dendrix}, referred to as $\mathrm{Dendrix}_{\mathrm{\textit{ILP}}}(k)$, discussed in \cref{multi-dendrix_chap3_first_sect};
    \item the MCMC algorithm, developed by \textcite{dendrix}, discussed in \cref{using_mcmc}.
\end{itemize}

When solving the MWSP, genes mutated in the same patients were grouped into \textit{metagenes}, as \textcite{multi-dendrix} did (details in \cref{results_multi-dendrix}). To evaluate the significance of the identified gene patterns, a permutation test was used. The results were reported for gene sets with sizes ranging between $2 \le k \le 10$, including an analysis of the second optimal patterns by removing the initially identified gene set, as \textcite{dendrix} did (details in \cref{results_dendrix}).

The three algorithms were first tested on data used by \textcite{dendrix} to compare performance: the ILP produced exact results in \textit{less than 1 second}, while the GA and the MCMC algorithm took over 60 and 5 seconds, respectively. Despite these differences in runtime, all three methods identified the same gene sets, such as the set (EGFR, KRAS, STK11) in the lung adenocarcinoma dataset, when $k = 3$ --- reported by \textcite{dendrix} too. Additionally, the BLP and GA methods were applied to three datasets not used by \textcite{dendrix}. As before, the ILP was able to consistently obtain exact results in \textit{under 1 second}, demonstrating its efficiency across multiple datasets. Details of the findings are listed below.

Stransky et al. (2011) performed whole-exome sequencing on 74 tumor-normal pairs, revealing previously unimplicated genes in \href{https://en.wikipedia.org/wiki/Head_and_neck_cancer}{head and neck squamous cell carcinoma} (HNSCC). The mutation dataset includes 4920 genes, with an average of 130 coding mutations per sample. This mutation matrix is sparse, and only TP53 and \href{https://en.wikipedia.org/wiki/Titin}{TTN} are mutated in more than 20 samples, affecting 46 and 23 samples, respectively. Therefore, to explore other pathways, the authors removed TP53 and TTN from the dataset and ran three methods on the remaining genes. When analyzing gene sets with k=6k=6, a unique optimal set (ANO4, CDKN2A, NFE2L2, NOTCH1, SYNE1, and TP63) was found, altered in 60.8\% of the samples with a p-value of 0.01. The optimal solutions for k<6k<6 were subsets of these six genes, while for k>6k>6, multiple optimal solutions were identified. The study suggests that the mutations in CDKN2A, NOTCH1, TP63, and SYNE1 are linked to terminal differentiation in squamous epithelia.
