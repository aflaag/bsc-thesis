\chapter{Finding driver pathways} \label{chap:finding_driver_pathways}

In the previous chapter, various studies were discussed in terms of how they \textit{formalized biological assumptions}, with particular emphasis on the metrics developed to assess \textbf{coverage} and \textbf{mutual exclusivity} within gene groups. This chapter will delve deeper into the algorithms employed by these studies to \textit{identify driver pathways} using their respective metrics and hypotheses.
...
\subsection{Using MCMCs} \label{using_mcmc}

To overcome the drawbacks of the aforementioned greedy algorithm, \textcite{dendrix} developed a \href{https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo}{\textit{Markov Chain Monte Carlo}} (MCMC) approach, which does not rely on assumptions about the distribution of mutation data or the number of patients. In particular, this MCMC method does not assume \textit{independence} among mutations in different genes, making it particularly useful for analyzing copy-number aberrations (CNAs), which often involve correlated mutations due to amplification or deletion of adjacent genes.
...
To begin, it is necessary to define two sets of indicator variables: consider a gene set $M$, described by a set of boolean variables, one for each gene $j \in M$, defined as follows
...
\begin{definition}[$\mathrm{Dendrix}_{\mathrm{\textit{ILP}}}(k)$]
    ...
    \begin{equation} \label{third_constr_dendrix_ilp}
        \sum_{j = 1}^n I_M(j) \cdot {a_{i, j}} \ge C_i(M), \quad 1 \le i \le m.
    \end{equation}
...
\begin{definition}[Multi-Dendrix]
    ...
    \begin{equation} \label{second_constr_multi-dendrix_ilp}
        \mathrm{subject \ to\ } \sum_{j = 1}^n I_{M_\rho}(j) \cdot {a_{i, j}} \ge C_i(M_\rho), \quad 1 \le i \le m,
    \end{equation}

    \begin{equation} \label{multi-dendrix_ilp_size}
        k_\mathrm{min} \le \sum_{j = 1}^n {I_{M_\rho}(j)} \le k_\mathrm{max}, \quad 1 \le \rho \le t,
    \end{equation}
...
\begin{itemize}
    ...
    \item \cref{multi-dendrix_ilp_last} forces each gene to appear in \textit{at most 1 set} within the collection, preventing \textit{overlappings} between sets.
\end{itemize}

Moreover, \textcite{multi-dendrix} state that this ILP can be extended to allow the gene sets of the collection to \textit{overlap}, since the genes in the intersection may be involved in multiple biological processes. Hence, \cref{multi-dendrix_ilp_last} is replaced with the following equation:
...
\begin{equation}
    \sum_{j = 1}^n {\sum_{\rho \neq \rho'} {{I_{M_\rho} (j) \cdot I_{M_{\rho'}} (j)}}} \le \tau, \quad 1 \le \rho \le t
\end{equation}
...
Since the greedy algorithm of Dendrix can identify a single driver pathway, finding multiple pathways could be achieved by running the algorithm \textbf{iteratively}. \textcite{multi-dendrix} provide a detailed explanation of this approach, referred to as Iter-Dendrix, with the pseudocode shown below.
...
This procedure runs the greedy algorithm iteratively, removing the chosen set from $\mathcal{G}$ after each iteration. \textcite{dendrix} discussed this approach toward the end of their work, highlighting some \textit{limitations}. In particular, if the gene sets corresponding to each pathway are \textit{disjoint}, Iter-Dendrix can be effective in identifying these sets, successfully finding disjoint sets $M_1$ and $M_2$ with high weight, as exclusivity is only evaluated within sets, not between them. However, if $M_1$ and $M_2$ \textit{share genes}, removing one set could also remove part of the other. In cases where the overlap is minimal, this approach may still identify the remaining portion of the second set. However, if the sets significantly intersect, Iter-Dendrix is likely to fail \cite{dendrix}.

\textcite{multi-dendrix} compare the outputs of their ILP with Iter-Dendrix: denoting with $M$ and $I$ the collections of gene sets obtained from Multi-Dendrix and Iter-Dendrix respectively, they state that $W'(M) \ge W'(I)$. They also argue that $M$ could contain sets with \textit{strictly greater weight} than the ones comprising $I$, due to several factors:
...
When applying the two algorithms to the BRCA dataset, at first the algorithms grouped frequently mutated genes into \textit{single sets}, despite their high coverage overlap. This was due to the weight function outweighing coverage $\abs{\Gamma(M)}$ over overlap $\omega(M)$. To enhance mutual exclusivity, \textcite{multi-dendrix} increased the overlap penalty, by using the following modified weight function: $$W(M) = \abs{\Gamma(M)} - \alpha \omega(M)$$ and a value of $\alpha = 2.5$. With this adjustment, Multi-Dendrix identified four distinct modules:

\begin{itemize}
    \item PI(3)K/AKT pathway: this module contains genes such as PTEN, PIK3CA, PIK3R1, \href{https://www.ncbi.nlm.nih.gov/gene/207}{AKT1}, and \href{https://www.ncbi.nlm.nih.gov/gene/64344}{HIF3A}, and amplification at 12p13.33; it is mutated in 61\% of samples, and it includes not only key genes in this pathway, but also the 12p amplification, though its target remains unclear;
...
The following section will discuss a method that utilizes the same scoring function $W(M)$, but it is based on a \textbf{genetic algorithm}.

\subsection{The genetic algorithm}

As outlined in the previous chapter, the weight function $W(M)$ has been \textit{widely adopted} across multiple studies, due to its intuitive nature and its suitability for formalizing \textit{mutual exclusivity} and \textit{coverage}. One study that employed this metric --- though not previously discussed, because its approach to mutual exclusivity mirrors that of \textcite{dendrix} --- is the work by \textcite{mdpfinder}, which introduced an algorithm called MDPFinder (\textit{knowledge-based} \cite{survey}).

Their method utilizes a \href{https://en.wikipedia.org/wiki/Genetic_algorithm}{\textit{Genetic Algorithm}} (GA), a flexible and adaptable approach capable of optimizing a wide range of scoring functions. It models genetic variation within a population, evolving through a process of random selection, thereby \textit{avoiding} the need to \textit{enumerate all possible solutions}.

Before detailing the genetic procedure, it is necessary to first define the \textbf{hypothesis space} and the \textbf{genetic operators} of the algorithm.
...
Therefore, the individual with the highest fitness value (i.e., \textit{highest ranking}) is most likely to be transferred into the next generation.
...
\begin{definition}[Crossover operator]
    The \textbf{crossover operator} specifies the breeding process as follows: the offspring \textit{inherits the variables shared} by both parents, while the non-shared ones are selected from the \textit{symmetric difference} of the parents' genetic makeup.
\end{definition}
...
To prevent premature convergence and enhance the accuracy of the algorithm, \textcite{mdpfinder} employ a \textit{local search strategy} to improve search performance, which is described below.
...
\begin{definition}[GA procedure]
    The following are the details of the \textbf{genetic algorithm}:
...
In practical applications, multiple optimal solutions may exist. Additionally, due to data noise and other factors, the solutions considered optimal --- i.e. the ones with the highest $W(M)$ --- may not necessarily be the most relevant in a biological context. To identify the most \textit{biologically meaningful} solutions, \textcite{mdpfinder} integrate other types of data, to refine the results. Specifically, the GA procedure is extended by incorporating \textbf{gene expression data} to enhance its performance.

The integrative model is developed based on the observation that genes within the same pathway typically collaborate to perform a specific function. Consequently, the expression profiles of gene pairs within the same pathway often exhibit \textit{higher correlations} than those in different pathways. This characteristic can be leveraged to distinguish between gene sets that have the \textit{same score}: the model focuses on detecting gene sets whose scores $W(M)$ are close to the optimal solution, but whose member genes display \textit{stronger correlations} with each other.
...
Moreover, note that $$-1 \le \mathrm{pcc}(x_{j_1}, x_{j_2}) \le 1 \implies 0 \le \abs{\mathrm{pcc}(x_{j_1}, x_{j_2})} \le 1 \implies 0 \le R(E_M) \le 1$$ therefore, when $\lambda = 1$ the value of $F_{ME}$ can be used to discriminate the gene sets with the same $W(M)$. Moreover, for values of $\lambda \ge 1$, the gene set with the \textbf{strongest correlation} and approximate exclusivity can be identified.

The next section will describe the results obtained by the GA algorithm using the integrative model, with $\lambda$ values set to 1 and 10.
...
The following section will explore the specifics of a different algorithm, that employs a \textbf{statistical approach} and a \textit{different scoring function}.

\subsection{A different greedy method}

The approach developed by \textcite{dendrix} employed a \textit{greedy algorithm} to search for the most mutually exclusive driver pathway (discussed in \cref{dendrix_first_sub}); this technique is highly versatile, as it can be adapted to a broad range of scoring functions. Similarly, \textcite{mutex} also utilized a \textbf{greedy algorithm} in their study, developing an algorithm called Mutex (\textit{knowledge-based} \cite{survey}), which employs a fundamentally \textit{different scoring function} than $W(M)$, which incorporates statistical and probabilistic elements.

The following algorithms will provide a \textit{partial} description of their approach; first, the main \textit{greedy procedure} is described below.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{expandGroup}{$M$, $G$, $A$, \texttt{final}}
            ...
            \State $b_s := 1$ \Comment{$b_s$ is $b$'s associated score}
...
Note that this procedure requires $c_s$ to improve \textbf{both} the current best score and $M$'s base score, meaning that \textit{suboptimal solutions} are not explored by the algorithm.
...
The complete algorithm works by calling \texttt{greedyMutex} for each possible $g \in \mathcal G$ with \texttt{final} set to \texttt{true}, and then comparing the resulting sets. Finally, these sets may optionally undergo a \href{https://en.wikipedia.org/wiki/False_discovery_rate}{false discovery rate} (FDR) control procedure, which lies outside the scope of this work. Note that many details from the \href{https://github.com/PathwayAndDataAnalysis/mutex}{original code} have been removed for brevity, in each described algorithm.
...
Note that $w^-$ depends solely on the mutual exclusivity component in each version of the algorithm, whereas the value of $w^+$ varies depending on the specific algorithm variant chosen. The following section will introduce the standard version of $\mathrm{C}^3$.
...
Note that in each version discussed, \textit{optional rescaling} is applied to ensure that the weight formulas satisfy additional constraints required later in the algorithm, though the specifics are beyond the scope of this analysis.
...
These findings indicate that network distance and connectivity information should be considered when identifying potential driver mutations. This can be achieved by adjusting the positive weight of edges connecting two genes: specifically, if both endpoint genes are drivers, they should be \textbf{sufficiently central} within a given pathway, close to other known drivers, or to each other.
...
The next section will describe a third variant, that incorporates \textit{gene expression data} instead of network information.

\subsection{Integrating expression data}

Another valuable type of data that could be integrated into the positive weights for clustering is \textbf{gene expression data}. This is based on the assumption that \textit{co-expressed genes} are likely to be involved in the \textit{same function} or cancer pathway. Therefore, genes with strong positive or negative co-expression should be \textit{clustered together}. The following paragraphs will describe how \textcite{c3} include expression data into $w^+$.
...
where $\abk{\mathrm{\mathbf a}, \mathrm{\mathbf b}}$ denotes the inner product of the vectors $\mathrm{\mathbf a}$ and $\mathrm{\mathbf b}$, while $\abs{\abs{\mathrm{\mathbf a}}}$ is $\mathbf{a}$'s $L^2$ norm. This equation is known as the \href{https://en.wikipedia.org/wiki/Cosine_similarity}{cosine similarity}, since the ratio that defines $g(u, v)$ is equal to the cosine of the angle between $\mathrm {\mathbf z(u)}$ and $\mathrm {\mathbf z(v)}$ --- the only difference is the absolute value in the numerator, which is used to capture both positive and negative correlations. A large value of $g(u, v)$ suggests that the expression vectors of $u$ and $v$ are \textbf{highly correlated}, hence they should be \textit{clustered together}. Note that $$\forall u, v \in V(G) \quad 0 \le g(u, v) \le 1$$ Moreover, let
...
\begin{definition}[EX-ME-CO]
    ...
    \begin{equation}
        \forall u, v \in V(G) \quad w_{uv}^+ := w_1 w_{uv}^+(\mathrm c) + w_2 w_{uv}^+(\mathrm x)
    \end{equation}
...
\textcite{c3} also mention that other combinations can be used, with appropriate adjustments to the weights, such as the following version, which will be termed NI-EX-ME-CO in this work.
...
\begin{definition}[NI-EX-ME-CO]
    ...
    \begin{equation}
        \forall u, v \in V(G) \quad w_{uv}^+ := w_1 w_{uv}^+(\mathrm c) + w_2 w_{uv}^+(\mathrm n) + w_3 w_{uv}^+(\mathrm x)
    \end{equation}
...
The previous paragraphs outlined the definitions of the \textit{weights} for the edges in the gene graph $G$; instead, this section will explain how $\mathrm{C}^3$ operates. Specifically, \textcite{c3} opted to use an ILP approach to formulate their \textbf{clustering algorithm}, which will be explained in more detail in the following paragraphs.

Note that the classical formulation of \textbf{correlation clustering} does not impose any restrictions on \textit{cluster sizes}. However, most driver identification methods inherently include cluster size limits, as they directly affect the computational complexity of the algorithms --- many even fail to operate beyond a certain size. Another reason for imposing a cluster size limit is the expectation that \textit{driver genes} of specific cancer types will be \textit{grouped together}, and recent findings indicate that only a small number of drivers are typically present in any given cancer type. Thus, if clusters are too large, they may include drivers from \textit{multiple cancer types}, hiding this separation of the drivers. Furthermore, introducing cluster size constraints helps to avoid the formation of non-informative \textit{giant clusters} or \textit{singleton clusters} \cite{c3}.

Therefore, \textcite{c3} introduce a cluster size constraint by assuming that all clusters are of size $k$ at most; clearly, setting $k$ equal to the total number of vertices effectively removes this constraint, allowing \textit{flexibility} in cluster size selection.
...
In this formulation, the variables $x_e$ allow to describe any \textit{non-overlapping} clustering of the vertices of $G$, since $x_e \in \{0, 1\}$ for each $e \in E(G)$.

Note that \cref{c3_first} aligns with the definitions provided in \cref{c3_chap2}, as $x_{uv} = 1$ implies that $u$ and $v$ should belong to \textit{different clusters}, while $x_{uv} = 0$ implies that the two vertices should be placed into the \textit{same cluster}.

Furthermore, \cref{c3_third} states that for a fixed vertex $u \in V(G)$, the number of variables $x_{uv}$ equal to 0, for any $v \in \mathcal N(u)$, must not exceed $k$ --- which is the clustering \textit{size constraint} previously discussed.
...
Since solving ILPs is \NPComplete \cite{karp}, \textcite{c3} relax the problem by changing \cref{c3_fourth} to an \textbf{interval constraint} $$0 \le x_e \le 1$$ leading to an LP program, the solution of which may be \textbf{fractional}. Hence, to obtain a valid clustering, the fractional solutions have to be \textit{rounded}. Therefore, instead of solving the LP, they remove \cref{c3_third} from the linear program, and employ the following \textbf{rounding procedure} to round the fractional values.

\begin{algorithm}[H]
    \caption{
        \textit{Rounding procedure}: given a solution $\{x_e\}_{e \in E(G)}$ of the \textit{relaxed version} of the ILP provided in \cref{c3_ilp}, a rational value $\alpha$, and the maximum cluster size $k$, the algorithm rounds the solution to integer values.
    }
...
The rationale behind the condition in line 7 can be explained by examining the meaning of $x_{uw}$: specifically, if $x_{uw}$ is close to 1, $u$ and $w$ are likely to be placed in \textit{different clusters}, as previously described in \cref{the_clustering_ilp}. Note that, if the sum $\sum_{w \in T}{x_{uw}}$ is greater than or equal to a value proportional to $\abs T$, it indicates that there are \textit{numerous edges} $(u, w)$ for $w \in T$ with \textit{significantly high} $x_{uw}$. Therefore, this suggests that $u$ should likely be placed in a cluster \textbf{different} from all its \textit{filtered} neighbors $T$.
...
\begin{itemize}
    \item to assess the \textit{mutual exclusivity} within a cluster, they evaluated the median pairwise exclusivity for each gene pair $(g_1, g_2)$ of the cluster, utilizing the Fisher's exact test --- specifically, they used the same contingency table described in \cref{contingency}, but in their case $M = \{g_1, g_2\}$;
    \item to quantify the \textit{coverage} of a given cluster, they calculated the proportion of patients who exhibited at least one alteration of a gene in the cluster;
    \item as mentioned in \cref{network_info}, driver genes tend to cluster closer together within biological pathways compared to random gene selections; therefore, to identify potential cancer driver genes, they measured the \textit{shortest network distances} between genes in the discovered clusters;
    \item lastly, to determine biological significance based on driver genes, they calculated the proportion of \textit{known drivers} within the ten most mutually exclusive clusters, using a curated list of driver genes from the CGC \cite{cgc}.
\end{itemize}
