...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{roundingProcedure}{$G$, $\{x_e\}_{e \in E(G)}$, $\alpha$, $k$}
            ...
            \State $S := V(G)$
            \While{$S \neq \varnothing$}
                \State $u \in S$ \Comment{this is the \textit{pivot vertex}}
                \State $T := \{w \in S - \{u\} \mid x_{uw} \le \alpha\}$ \Comment{$u$'s neighbors under $\alpha$'s threshold}
                \If{$\sum_{w \in T}{x_{uw}} \ge \frac{\alpha}{2}\abs{T}$} \Comment{TODO ADD COMMENT}
                    \State $\mathcal C = \mathcal C \cup \{u\}$ \Comment{TODO ADD COMMENT}
                    \State $S = S - \{u\}$
                \ElsIf{$\abs{T} \le k$}
                    \State $\mathcal C = \mathcal C \cup \rbk{\{u\} \cup T}$
                    \State $S = S - \rbk{\{u\} \cup T}$
                \Else
                    \State TODO DA FINIRE E AGGIUNGERE COMMENTI
                    \State $S = S - \rbk{\{u\} \cup T}$
                \EndIf
            \EndWhile
...
\begin{definition}[C3] \label{c3_final}
    The \textbf{C3 algorithm} is defined as follows: first, this ILP is solved
    ...
    then, the rounding procedure defined in \cref{rounding_procedure} is applied.
