...
\begin{algorithm}[H]
    \caption{
        ...
        \label{greedy_mutex}
    \begin{algorithmic}[1]
        \Function{greedySearch}{$g$, $k_\mathrm{max}$, $G$, $A$, \texttt{final}}
            \State $M := \{g\}$
            \Do
                \State $M_p := M$
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{expandGroup}{$M$, $G$, $k_\mathrm{max}$, $A$, \texttt{final}}
            \State $\mathcal C := \texttt{getCandidates}(M, G)$ \Comment{TODO}
            ...
            \For{$c \in \mathcal C$}
                \If{\texttt{final}}
                    ...
                    \For{$g \in (M \cup \{c\})$}
                        \State $\mathcal N \texttt{[}g\texttt{]} = \mathcal N_g'$ \Comment{$\mathcal N_g'$ is $g$'s null distribution, based on $\mathcal P' \texttt{[} g\texttt{]}$}
                    \EndFor
                \EndIf
