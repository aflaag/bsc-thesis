...
We applied our MCMC algorithm to analyze somatic mutations obtained from high-throughput genotyping of 238 oncogenes across 1,000 patients spanning 17 cancer types (Thomas et al. 2007). Additionally, we applied the algorithm to somatic mutations identified in recent cancer sequencing studies of lung adenocarcinoma (Ding et al. 2008) and glioblastoma multiforme (The Cancer Genome Atlas Research Network 2008). In the glioblastoma multiforme analysis, both copy-number aberrations and single-nucleotide (or small indel) mutations were included, whereas, in the lung adenocarcinoma analysis, we focused solely on single-nucleotide (or small indel) mutations. The MCMC algorithm samples gene sets with a frequency proportional to their weights; therefore, to narrow down to high-weight sets, we report only those with a frequency of at least 1\%. We further optimized the mutation matrix by merging genes mutated in the exact same patients into larger "metagenes." \todo{da riscrivere}
