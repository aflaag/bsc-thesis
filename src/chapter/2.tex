...
The first part of this chapter discusses the \textit{two categories} of mutations that occur in cancer, one of which is significantly more influential in tumor development. Distinguishing between these categories is essential for understanding cancer mechanisms, as the most lethal mutations may serve as critical targets for future targeted therapies.

However, classifying cancerous mutations is not straightforward and poses several challenges, which will be outlined in the following sections. To address these difficulties, research has increasingly focused on identifying patterns within patients' genomic data for further analysis.

% However, classification of cancerous mutations is not straightforward, and poses several challenges, which will be detailed in the following sections. To address these difficulties, research has increasingly focused on finding some pattern within patients' genomic data to leverage.

Recent analyses of genomic data from numerous patients have revealed patterns that may lead to significant therapeutic discoveries, and assist in categorizing mutations into the two described types. The second half of this chapter introduces \textit{four} different approaches aimed at identifying cellular signaling and regulatory pathways critical to cancer development. The focus will be on the metrics developed by these approaches to assess patterns in patient data.
