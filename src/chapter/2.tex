...
% ------- TODO USE THIS
% estimates suggest that the human genome contains more than 50,000 genes \cite{n-genes}

placeholder \todo{introduction to the chapter, tell why we have multiple algorithms (use comment), also change chapter name?}
...
Indeed, $\omega(M)$ is the \textit{number of patients that are counted more than once in the sum}. Note that $\omega(M) \ge 0$, with equality holding only if no patient has more than one mutated gene of $M$.
...
Finding the solution to this problem is computationally difficult, even for small values of $k$ (e.g. there are $\approx 10^{23}$ subsets of size $k = 6$ of 20,000 genes). In fact, the following proof (provided by \textcite{dendrix}) shows that this problem is NP-Hard.

\begin{theorem}[NP-Hardness of the MWSP] \todo{turnalo in NP-completezza e dimostra che la riduzione è polinomiale}
...
%%% TODO QUESTO COMMENTO VA NEL QUARTO CAPITOLO
% These weights are calculated using four types of datasets: gene mutation data, \href{https://www.genome.gov/genetics-glossary/Copy-Number-Variation}{copy number variation (CNV)}, network information, and gene expression data. To appropriately combine the sources from which the information was obtained, linear combinations were utilized to account for the reliability of the sources from which the data was drawn.
