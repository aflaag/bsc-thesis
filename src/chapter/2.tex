...
\subsection{A deterministic equation}
...
Note that $W(M) = \Gamma(M)$ when $M$ is mutually exclusive \todo{questa cosa l'ho scritta io pensando che dovesse essere $= 0$ strettamente}. In order to find an optimal gene set, the following problem has to be solved:
...
\subsection{A clustering approach}
