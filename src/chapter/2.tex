...
and Fisher proved that the following equality holds $$P(X = \abs{\Gamma(g)}) = \dfrac{m!m!\abs{\Gamma(M)}!(2m - \abs{\Gamma(M)})!}{(2m)!  \abs{\Gamma(g)}! \abs{\Gamma(M - \{g\})}!  (m - \abs{\Gamma(g)})! (m - \abs{\Gamma(M - \{g\})})!}$$
