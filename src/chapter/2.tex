...
The author's of a very well-known paper, which developed two algorithms called \curlyquotes{Dendrix} \cite{dendrix}, gave the following mathematical formalization to the properties of mutual exclusivity and coverage for a set of genes.
...
Note that $\mathscr{S}(u)$ corresponds to $\Gamma(u)$ under Dendrix's notation, but defined through the $M$ matrix respectively.

\begin{definition}[Mutual exclusivity component]
    The \textbf{mutual exclusivity component} is defined as follows: $$w_{uv}^-(\mathrm e) := a \cdot \dfrac{\abs{\mathscr{S}(u) \cap \mathscr{S}(v)}}{\min(\abs{\mathscr{S}(u)}, \abs{\mathscr{S}(v)})}$$ where $a$ is a user-defined scaling parameter.
\end{definition}
...
\begin{definition}[Negative weights]
    \textbf{Negative weights} only depend on the mutual exclusivity component, i.e. $$w_{uv}^- := w_{uv}^-(\mathrm e)$$
\end{definition}
...
\begin{definition}[Coverage component]
